...
interface CaseDecisionFormProps {
  caseId: number;
...
export default function CaseDecisionForm({ caseId, onDecisionSubmitted }: CaseDecisionFormProps) {
  const { user } = useAuth();
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Provide the final decision for Case #{caseId}. This will be visible to the customer once marked as Final or Communicated.
            </DialogDescription>
          </DialogHeader>
...
// Component to display case decisions
...
interface CaseDecisionListProps {
  decisions: CaseDecision[];
  showInternalNotes?: boolean;
}

export function CaseDecisionList({ decisions, showInternalNotes = false }: CaseDecisionListProps) {
  if (decisions.length === 0) {
    return (
      <p className="text-sm text-muted-foreground py-2">No decisions yet.</p>
    );
  }
        ...
        return (
          <div key={decision.decision_id} className="p-3 bg-muted/50 rounded-lg border space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Badge className={cat?.color || ''}>{cat?.label || decision.category}</Badge>
              <Badge className={stat?.color || ''} variant="outline">{stat?.label || decision.status}</Badge>
            ...
            {showInternalNotes && decision.internal_notes && (
